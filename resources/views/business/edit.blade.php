...
<div class="row">
    ...
    <div class="col-md-6">
        ...
        <div class="card mb-3">
            ...
            <form action="{{ route('edit_business_logo_action', $edit_business->business_id) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <div class="">
                                ...
                                <img src="{{env('UPLOAD_URL').$edit_business->file_path}}" alt="" width="100px" class="mt-2">
                            </div>
                            @error('logo')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
...
<script>
    $(document).ready(function() {
        $('#check-business').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('check_new_business') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    business_id: "{{ $edit_business->business_id }}",
                    business_name: $('#business_name').val(),
                    business_website: $('#business_website').val()
                },
                success: function(res) {
                    if (res.status === 'error') {
                        if (res.errors.business_name) {
                            $('#caption-error').text(res.errors.business_name).removeClass('d-none');
                        }
                        if (res.errors.url) {
